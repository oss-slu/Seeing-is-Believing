...
const Content = (props) => {
  ...
  const seeGrades = (class_id) => {
	router.push(`/student/grades?cl=${class_id}`)
};
  ...
  return (
		<>
			{languages && semesters ? (
				<Card>
					<Scrollbar>
						<Table sx={{minWidth: 600, borderRadius:'10px'}}>
							<TableHead>
								<TableRow>
										...
										>
											<MoreMenu
												...
												options={[
													{
														display:"See Grades",
														link: `/student/grades?cl=${item.id}`,
													}
												]}
											/>
										</TableCell>
